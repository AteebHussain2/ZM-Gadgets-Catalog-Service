...
export function ImageGallery({ images, productName, hasDiscount, discountPct }: ImageGalleryProps) {
    ...
    const currentImage = images[selectedIndex] || `/placeholder.svg?height=800&width=800&query=${encodeURIComponent(productName)}`
                ...
                <div className="flex gap-2 overflow-x-auto pb-2">
                    {images.map((url, index) => (
                        <button
                            ...
                            className={`relative h-20 w-20 shrink-0 overflow-hidden rounded-lg border-2 transition-all ${index === selectedIndex
                        ...
                        >
                            ...
                            />
                            {index === selectedIndex && (
                                <div className="absolute inset-0 bg-[#F7941D]/10" />
                            )}
                        </button>
                    ))}
                </div>
                ...
                <div className="flex justify-center gap-1 sm:hidden">
                    {images.map((_, index) => (
                        <button
                            key={index}
                            onClick={() => setSelectedIndex(index)}
                            className={`h-2 w-2 rounded-full transition-all ${index === selectedIndex ? "bg-[#F7941D]" : "bg-muted-foreground/30"
                                }`}
